 ...
 *
 *   AlternateMiningDrones-<version>-EveJS-<evejs>.zip
 *     README.md CHANGELOG.md MANUAL.md
 *     install.bat uninstall.bat status.bat update.bat
 *     install.js uninstall.js update.js lib/  <- installer
 *     AlternateMiningDrones/                       <- payload it copies into mods/
 *
 *   AlternateMiningDrones-<version>-EveJS-<evejs>-launcher.zip
 *     AlternateMiningDrones/                       <- the same folder, alone
 *
...
// Release notes and the user manual ride at the archive root as well, so whoever unpacks the ZIP
// reads them without opening the mod folder. Both come from the mod root: installer/README.md is
// the installer's own document and is deliberately not the mod's README.
const ROOT_DOCUMENTS = Object.freeze(["CHANGELOG.md", "MANUAL.md"]);
...
function main() {
  ...
  const installerEntries = [
    ...INSTALLER_FILES.map((relative) => ({
      name: relative,
      data: fs.readFileSync(path.join(INSTALLER_DIR, ...relative.split("/"))),
    })),
    ...ROOT_DOCUMENTS.map((relative) => ({ name: relative, data: fs.readFileSync(path.join(MOD_DIR, relative)) })),
  ];
