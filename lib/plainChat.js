...
// Consumes the "!atm ..." line out of ordinary chat.
//
...
const OVERLAY_MARKER = Symbol.for("evejs.alternateMiningDrones.plainChat");
const FALLBACK_MESSAGE = "AlternateMiningDrones: the command produced no reply.";
...
function install(upstream, options = {}) {
  ...
  for (const channel of CHANNELS) {
    ...
    const wrapped = function alternateMiningDronesPlainChat() {
      ...
      try {
        ...
        message = `AlternateMiningDrones: the command failed (${error && error.message}).`;
      }
  ...
  if (hooked.length === 0) {
    logError(
      ...
      "the plain-chat !trigger is unavailable and only /atm works",
    );
    return upstream;
  }
