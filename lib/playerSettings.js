...
const FILE_HELP =
  ...
  "one name may hold a space - \"m filter add gneiss, dark ochre\" is two entries. A kind the " +
  ...
  "foreign: off|warn|allow - whose wrecks may be worked: only your own, another player's " +
  "with a warning in chat, or another player's silently. " +
...
// Per kind of drone. A kind lives under its own key, so /aud m off and /aud s off
...
const SALVAGE_FIELDS = Object.freeze({
  ...
  choices: Object.freeze({
    ...
    foreign: Object.freeze(["off", "warn", "allow"]),
  }),
});
...
// A stored entry, in the shape 1.0.0-alpha writes: shared keys at the top level, then
