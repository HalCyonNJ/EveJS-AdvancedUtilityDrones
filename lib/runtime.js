...
const MOD_VERSION = "1.0.2-beta.1";
...
function createRuntime(options) {
  ...
  function orderedSalvageCandidates(
    allowed, claims, settings, farFirst, claimPenalty, allowClaimed = false,
  ) {
    let candidates = allowed;
    if (!allowClaimed && settings.targetMode !== TARGET_MODE_FOCUS) {
      const unclaimed = allowed.filter((entry) => (
        toInt(claims.get(entry.targetID), 0) <= 0
      ));
  ...
  function runSalvageShipPass(scene, controllerID, shipDrones, nowMs, claims) {
    ...
    const salvagerRuntime = safeSalvagerRuntime();
    const activeTargetID = (droneEntity) => {
    ...
    // Active drones are claims too. Otherwise a later idle drone sees an
    // empty map and chooses the wreck another drone is already working.
    for (const drone of managedDrones) {
      addSalvageClaim(claims, activeTargetID(drone), 1);
    }

    const tryAssign = (drone, allowClaimed, retarget) => {
      const candidates = orderedSalvageCandidates(
        allowed, claims, settings, farFirst, claimPenalty, allowClaimed,
      );
    ...
    const assignByMode = (drone, retarget) => {
      if (settings.targetMode === TARGET_MODE_FOCUS) {
        return tryAssign(drone, true, retarget);
      }
      const strict = tryAssign(drone, false, retarget);
