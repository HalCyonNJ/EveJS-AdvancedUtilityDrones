...
function createServerDeps(runtimeRoot) {
  ...
  return Object.freeze({
    ...
    // The game's own answer to "may this pilot take from this wreck", the same
    // module the loot path asks before a single item moves.
    getSpaceLootEntitlement: () => (
      load("server/src/services/_shared/spaceLootEntitlement.js")
    ),
...
function createRuntime(options) {
  ...
  const stats = {
    ...
    retargets: 0,
    // Salvage: orders issued, recalls caused by salvage, and the warning lines
    // printed about somebody else's wreck.
  ...
  function getSalvageState(characterID) {
    ...
    return {
      ...
      distance: pick("distance", config.salvageDistance) === SALVAGE_DISTANCE_FARTHEST
        ? SALVAGE_DISTANCE_FARTHEST
        : "nearest",
      foreign: [SALVAGE_FOREIGN_OFF, SALVAGE_FOREIGN_WARN, SALVAGE_FOREIGN_ALLOW]
        .includes(pick("foreign", config.salvageForeign))
        ? pick("foreign", config.salvageForeign)
        : SALVAGE_FOREIGN_OFF,
      ...sharedPlayerValues(overrides),
      source: overrides ? "player" : "config",
    };
  ...
  function holdDestinationFlags(storage, state) {
    ...
    // resolveCargoSnapshot, so a hull that has a dedicated salvage hold still
    // fills its cargo hold - and a guard that watched the salvage hold instead
    // would recall at the wrong moment. Follow the server, not the hold layout.
  ...
  function resumeDrones(session, asked = null) {
    ...
    // Drones coming back out of the bay start clean: the warning about a wreck
    // they are sent to is printed again on the next launch. This is the launch
    // path the loader calls, so "every time the player puts the drones out
    // again" is exactly when the warning comes back.
    const askedKind = asked === null || asked === undefined ? null : String(asked);
    if (askedKind !== "mining") {
      clearSalvageWarnings(resolveControllerCharacterID(shipEntity));
    }
    let resumed = 0;
  ...
  // ─── Salvage ───────────────────────────────────────────────────────────────
  //
  // A wreck is not a rock. It belongs to somebody, it is worth something to
  // them, and whether this pilot may touch it is a question the game already
  // answers elsewhere: the loot path asks it before a single item moves. The
  // clearance this mod flies on is that same answer, so an abandoned wreck, a
  // corporation's wreck and a stranger's wreck are treated exactly as the game
  // treats them, and the safety light is honoured rather than ignored.
  //
  // What the game does *not* do is stop a manual salvage: droneRuntime's
  // commandSalvage never consults the entitlement check, so on this server a
  // pilot on a green light can already salvage a stranger's wreck by hand and
  // walk away with no suspect timer. The guard below is therefore the mod's own
  // policy rather than a mirror of something already there, and the warning it
  // prints says so.

  // Wreck IDs this pilot has been warned about since the last launch. The
  // warning is per launch, not per scan: the operator's words for the rule are
  // "every time the player puts the drones out again".
  const salvageWarnedWrecks = new Map();

  function noteSalvageWarning(characterID, wreckID) {
    const key = toInt(characterID, 0);
    const id = toInt(wreckID, 0);
    if (key <= 0 || id <= 0) {
      return false;
    }
    let seen = salvageWarnedWrecks.get(key);
    if (!seen) {
      seen = new Set();
      salvageWarnedWrecks.set(key, seen);
    }
    if (seen.has(id)) {
      return false;
    }
  ...
  // A fresh launch is a fresh start: the next time those drones go out, the
  // warning about a wreck they are sent to is printed again.
  function clearSalvageWarnings(characterID) {
    const key = toInt(characterID, 0);
    if (key > 0) {
      salvageWarnedWrecks.delete(key);
    }
  }
  ...
  // May this pilot take from this wreck, and would taking it flag them? Both
  // answers come from the game's own entitlement check, so corporation and fleet
  // loot rights, a wreck that was explicitly abandoned and "the owner is not a
  // player character" all behave the way they behave everywhere else. The known flag is
  // false when that check could not be reached at all, which is the one case the
  // caller has to decide about rather than trust.
  function evaluateWreckAccess(session, scene, entity) {
    const source = {
    ...
    try {
      const entitlement = deps.getSpaceLootEntitlement
        ? deps.getSpaceLootEntitlement()
        : null;
      if (!entitlement || typeof entitlement.evaluateSpaceLootAccess !== "function") {
        return {
          entitled: false, blocked: false, requiresSuspectTimer: false,
          ownerID: source.ownerID, known: false,
        };
      }
      source.lootInfo = typeof entitlement.readSpaceLootInfo === "function"
        ? entitlement.readSpaceLootInfo(entity && entity.customInfo)
        : {};
      const result = entitlement.evaluateSpaceLootAccess(session, source, { scene }) || {};
      return {
        entitled: result.entitled === true,
        blocked: result.success !== true,
        requiresSuspectTimer: result.requiresSuspectTimer === true,
        ownerID: source.ownerID,
        known: true,
      };
    } catch (error) {
      logError("wreck entitlement check failed: " + (error && error.message));
      return {
        entitled: false, blocked: false, requiresSuspectTimer: false,
        ownerID: source.ownerID, known: false,
      };
  ...
  // The warning the operator asked for: a line in local chat, once per wreck per
  // launch. The suspect flag this warns about is public in the game anyway, so
  // the local channel is where it belongs - and it is the only text channel a
  // tick has, because the private system line the command replies use is produced
  // by the vendor's own catch on the player-message path, which is not on the
  // stack here.
  function hasSalvageWarning(characterID, wreckID) {
    const seen = salvageWarnedWrecks.get(toInt(characterID, 0));
    return Boolean(seen && seen.has(toInt(wreckID, 0)));
  }

  // `reason` is "safety" when the game's own loot rules refuse the wreck and
  // "foreign" when the mod's salvage policy does. Either way the drones stay off
  // that wreck, and the line names the one switch that would change the answer.
  function warnAboutWreck(characterID, wreckID, access, reason) {
    // Checked before the send and recorded after it, so a warning that could not
    // reach anybody yet is still printed the first time it can. A wreck goes on
    // the list only once the line is actually out.
    if (hasSalvageWarning(characterID, wreckID)) {
      return false;
    }
    const owner = salvageOwnerLabel(access.ownerID);
    const text = reason === "safety"
      ? "AdvancedUtilityDrones warning: wreck " + wreckID + " belongs to " + owner +
        " and your safety light will not allow the drones to touch it. Set the light to " +
        "yellow to work it, or leave it alone."
      : "AdvancedUtilityDrones warning: wreck " + wreckID + " belongs to " + owner +
        " and the salvage drones leave other pilots' wrecks alone. \"/aud s foreign " +
        "allow\" works it anyway and takes the suspect flag.";
    let sent = false;
    try {
      const chatRuntime = deps.getChatRuntime();
      if (chatRuntime && typeof chatRuntime.broadcastLocalMessage === "function") {
        const session = deps.getSessionRegistry().findSessionByCharacterID(characterID);
        if (session) {
          chatRuntime.broadcastLocalMessage(session, text);
          sent = true;
        }
      }
    } catch (error) {
      logError("salvage warning could not be sent: " + (error && error.message));
    }
    if (sent) {
      noteSalvageWarning(characterID, wreckID);
      stats.salvageWarnings += 1;
    }
    return sent;
  }
  ...
  function runSalvageShipPass(scene, controllerID, shipDrones, nowMs, claims) {
    ...
    const configuredRange = Number.isFinite(settings.rangeOverrideMeters)
      ...
      : (rangeInfo && rangeInfo.rangeMeters) || config.baseRangeMeters;
    const wrecks = listSceneWrecks(scene, controllerEntity, configuredRange);
    if (wrecks.length === 0) {
      return;
    }
    // One entitlement question per wreck per pass, not one per drone: with a
    // hull that launches 50 drones the per-drone answer would be the same 50
    // times over, and the check reaches into character, fleet and crimewatch
    // state.
    const allowed = [];
    for (const wreck of wrecks) {
      const access = evaluateWreckAccess(session, scene, wreck.entity);
      if (access.blocked) {
        warnAboutWreck(characterID, wreck.targetID, access, "safety");
        continue;
      }
      if (!access.entitled) {
        if (settings.foreign === SALVAGE_FOREIGN_OFF) {
          continue;
        }
        // "warn" is the middle rung, and it stops the drones rather than only
        // narrating them: the pilot is told which wreck was passed over and which
        // switch opens it. "allow" is the only setting that works another pilot's
        // wreck, and it is the one that flags them, so it is never the default.
        if (settings.foreign === SALVAGE_FOREIGN_WARN) {
          warnAboutWreck(characterID, wreck.targetID, access, "foreign");
          continue;
        }
      }
      allowed.push({ ...wreck, access });
    }
    if (allowed.length === 0) {
      return;
    }
    ...
    for (const drone of idleDrones) {
      const scored = allowed
        .map((entry) => {
          ...
          return {
            entry,
            // A wreck this pilot is entitled to beats one that would flag them,
            // whatever the distance rule says: the free ones are taken first.
            trust: entry.access.entitled ? 0 : 1,
      ...
      for (const candidate of scored) {
        ...
        if (config.verbose) {
          log(
            ...
            (candidate.entry.access.entitled ? "" : " (another pilot's wreck)"),
          );
        }
        break;
      }
  ...
  function describeSalvageStatus(session) {
    ...
    return {
      ...
      workingDrones,
      idleDrones,
      wrecks: { total: wrecks.length, mine, theirs, blocked },
      range: rangeInfo,
      rangeMeters,
      hold,
      playersFile: players && typeof players.file === "function" ? players.file() : null,
  ...
  // and whether this pilot may work it. Backs "/aud s list".
  function describeSalvageWrecks(session, limit = 12) {
    ...
    for (const wreck of wrecks.slice(0, wanted)) {
      ...
      entries.push({
        ...
        owner: salvageOwnerLabel(access.ownerID),
  ...
  return Object.freeze({
    ...
    // Whose wrecks the salvage drones may work. "off" is own wrecks only, which
    // is what the game's own auto-salvage does; "warn" and "allow" open it up to
    // another pilot's wreck, with a warning line in chat for "warn".
    setPlayerSalvageForeign(characterID, foreign, meta = null) {
      const accepted = [SALVAGE_FOREIGN_OFF, SALVAGE_FOREIGN_WARN, SALVAGE_FOREIGN_ALLOW];
      return updatePlayerSettings(characterID, {
        salvage: {
          foreign: accepted.includes(foreign) ? foreign : SALVAGE_FOREIGN_OFF,
        },
      }, meta, getSalvageState);
    },
