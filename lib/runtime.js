...
// bucket of its own: /aud mining filter ore never matches a moon rock. Returns null for
...
function createServerDeps(runtimeRoot) {
  ...
  return Object.freeze({
    ...
    getFleetRuntime: () => load("server/src/services/fleets/fleetRuntime.js"),
...
function createRuntime(options) {
  ...
  // getPlayerState on purpose: the two kinds are separate switches, so /aud mining off
  // leaves the salvage drones working and /aud salvage off leaves the miners working.
  ...
  function addMiningClaim(claims, targetID, delta) {
    const id = toInt(targetID, 0);
    if (id <= 0) {
      return;
    }
    const next = toInt(claims.get(id), 0) + delta;
    if (next > 0) {
      claims.set(id, next);
    } else {
      claims.delete(id);
  ...
  // The salvage target mode and distance are part of what the drones are asked
  // to do, so a change reaches drones that are already working.
  const lastSalvageSettingSignature = new Map();

  function noteSalvageSettingSignature(characterID, settings) {
    const signature = `${settings.targetMode}|${settings.distance}`;
    const previous = lastSalvageSettingSignature.get(characterID);
    lastSalvageSettingSignature.set(characterID, signature);
  ...
  // for it with /aud mining resume, and the loader calls it after a launch so a
  ...
  function runShipPass(scene, controllerID, shipDrones, nowMs, claims) {
    ...
    for (const [kind, drones] of dronesByKind) {
      ...
      for (const drone of drones) {
        ...
        for (const entry of scored) {
          ...
          addMiningClaim(claims, targetID, 1);
          if (skipTargetID > 0 && skipTargetID !== targetID) {
            addMiningClaim(claims, skipTargetID, -1);
          }
          stats.assignments += 1;
          if (skipTargetID > 0) {
            stats.retargets += 1;
    ...
    if (blockedRetargets.length > 0) {
      for (const drone of blockedRetargets) {
        addMiningClaim(
          claims,
          retargetTargets.get(toInt(drone && drone.itemID, 0)) || 0,
          -1,
        );
      }
  ...
  // The raw list has no distance attached: one fleet scan is reused by every
  // ship, and each ship still orders the wrecks from its own position.
  function scanSceneWreckEntities(scene) {
    let entities = [];
    try {
    ...
    for (const entity of entities) {
      ...
      if (targetID > 0 && !byID.has(targetID)) {
        byID.set(targetID, entity);
      }
    }
    return [...byID.values()];
  }

  function cachedSceneWreckEntities(scene, passContext, scopeKey) {
    if (!passContext || !(passContext.salvageScans instanceof Map)) {
      return scanSceneWreckEntities(scene);
    }
    const key = scopeKey || "scene";
    if (!passContext.salvageScans.has(key)) {
      passContext.salvageScans.set(key, scanSceneWreckEntities(scene));
    }
    return passContext.salvageScans.get(key);
  }

  function listSceneWrecks(
    scene, shipEntity, maxRangeMeters, wreckEntities = null, farFirst = false,
  ) {
    const entities = Array.isArray(wreckEntities)
      ? wreckEntities
      : scanSceneWreckEntities(scene);
    const found = [];
    for (const entity of entities) {
    ...
    found.sort((left, right) => (
      (farFirst ? right.distance - left.distance : left.distance - right.distance) ||
  ...
  function currentSalvageTargetID(droneEntity) {
    const salvage = droneEntity && droneEntity.droneSalvage;
    return toInt(
      salvage && salvage.targetID,
      toInt(droneEntity && droneEntity.targetID, 0),
    );
  }

  function isAutomatedSalvageDrone(droneEntity, settings) {
    if (!droneEntity || droneEntity.droneCommand !== DRONE_COMMAND_SALVAGE) {
      return false;
    }
    if (droneEntity.droneAssist) {
      return false;
    }
    if (toInt(droneEntity.advancedUtilityDronesSalvageTargetID, 0) <= 0) {
      return false;
    }
    return !isPlayerParked(droneEntity, settings);
  }

  function activeSalvageTargetID(
    scene, controllerEntity, droneEntity, salvagerRuntime,
  ) {
    const targetID = currentSalvageTargetID(droneEntity);
    if (targetID <= 0) {
      return 0;
    }
    const targetEntity = scene && scene.getEntityByID(targetID);
    if (!targetEntity || targetEntity === controllerEntity) {
      return 0;
    }
    return isSalvageableWreckEntity(targetEntity, salvagerRuntime) ? targetID : 0;
  }

  // The common spread/nearest/farthest path consumes the distance-ordered list
  // with one cursor. Sorting the whole field is reserved for the fallback below,
  // which only runs when every wreck is already claimed.
  function createSalvageCursor(allowed, claims) {
    let index = 0;
    return {
      reset() {
        index = 0;
      ...
      nextUnclaimed() {
        while (index < allowed.length) {
          ...
          index += 1;
          if (toInt(claims.get(entry.targetID), 0) <= 0) {
            return entry;
          }
        }
        return null;
  ...
  function bestClaimedSalvageCandidate(
    allowed, claims, settings, farFirst, claimPenalty, excluded = null,
  ) {
    let best = null;
    let bestScore = Number.POSITIVE_INFINITY;
    let bestTargetID = Number.POSITIVE_INFINITY;
    for (const entry of allowed) {
      if (excluded && excluded.has(entry.targetID)) {
        continue;
      }
      ...
      const score = (farFirst ? -entry.distance : entry.distance) + penalty;
      if (
        score < bestScore ||
        (score === bestScore && entry.targetID < bestTargetID)
      ) {
        best = entry;
        bestScore = score;
  ...
  function addSalvageClaim(claims, targetID, delta) {
    if (targetID <= 0) {
      return;
    }
    const next = toInt(claims.get(targetID), 0) + delta;
    if (next > 0) {
  ...
  function runSalvageShipPass(
    scene, controllerID, shipDrones, nowMs, claims, passContext, scopeKey,
  ) {
    const controllerEntity = scene.getEntityByID(controllerID);
    if (!controllerEntity) {
      return;
    ...
    const managedDrones = salvageDrones.filter((drone) => (
      isAutomatedSalvageDrone(drone, settings)
    ));
    if (idleDrones.length === 0 && managedDrones.length === 0) {
      return;
    }
    const settingsChanged = noteSalvageSettingSignature(characterID, settings);
    const salvagerRuntime = safeSalvagerRuntime();
    const activeTargetID = (droneEntity) => activeSalvageTargetID(
      scene, controllerEntity, droneEntity, salvagerRuntime,
    );
    const needsAssignment = settingsChanged || idleDrones.length > 0 ||
      managedDrones.some((drone) => {
        const currentID = activeTargetID(drone);
        const expectedID = toInt(drone.advancedUtilityDronesSalvageTargetID, 0);
        return currentID <= 0 || currentID !== expectedID;
      });
    // This is the expensive-path gate: a squadron that is already working valid
    // wrecks never asks the scene for its visible-entity list.
    if (!needsAssignment) {
      return;
    }
    const session = deps.getSessionRegistry().findSessionByCharacterID(characterID);
    ...
    const wreckEntities = cachedSceneWreckEntities(scene, passContext, scopeKey);
    const allowed = listSceneWrecks(
      scene, controllerEntity, configuredRange, wreckEntities, farFirst,
    );
    if (allowed.length === 0) {
      return;
    }
    ...
    const droneRuntime = deps.getDroneRuntime();
    const claimPenalty = Math.max(0, toFiniteNumber(config.claimPenaltyMeters, 0));
    const salvageCursor = createSalvageCursor(allowed, claims);
    const holdState = { yieldKind: "salvage", unitVolume: SALVAGE_UNIT_VOLUME_M3 };
    const tryCandidate = (drone, candidate, retarget) => {
      const targetEntity = candidate.entity;
      ...
      addSalvageClaim(claims, targetID, 1);
      drone.advancedUtilityDronesSalvageTargetID = targetID;
      stats.salvageAssignments += 1;
      stats.lastSalvageAssignAtMs = nowMs;
      scheduleStateRefresh(drone, nowMs);
      if (config.verbose) {
        log(
          (retarget ? "reassigned" : "assigned") +
          " salvage drone " + toInt(drone.itemID, 0) + " to wreck " + targetID +
          " at " + Math.round(candidate.distance) + " m",
        );
      }
      return "assigned";
    };

    const tryAssign = (drone, allowClaimed, retarget, tried) => {
      for (;;) {
        let candidate = null;
        if (!allowClaimed && settings.targetMode !== TARGET_MODE_FOCUS) {
          do {
            candidate = salvageCursor.nextUnclaimed();
          } while (candidate && tried.has(candidate.targetID));
        } else {
          candidate = bestClaimedSalvageCandidate(
            allowed, claims, settings, farFirst, claimPenalty, tried,
          );
        }
        if (!candidate) {
          return "none";
        }
        tried.add(candidate.targetID);
        const result = tryCandidate(drone, candidate, retarget);
        if (result === "assigned" || result === "stop") {
          return result;
        }
      }
    };

    const assignByMode = (drone, retarget) => {
      const tried = new Set();
      if (settings.targetMode === TARGET_MODE_FOCUS) {
        return tryAssign(drone, true, retarget, tried);
      }
      const strict = tryAssign(drone, false, retarget, tried);
      if (strict === "assigned" || strict === "stop") {
        return strict;
      }
      return tryAssign(drone, true, retarget, tried);
    };

    for (const drone of managedDrones) {
      const currentID = activeTargetID(drone);
      const expectedID = toInt(drone.advancedUtilityDronesSalvageTargetID, 0);
      if (!settingsChanged && currentID > 0 && currentID === expectedID) {
        continue;
      }
      const rawTargetID = currentSalvageTargetID(drone);
      const rawTargetEntity = rawTargetID > 0 ? scene.getEntityByID(rawTargetID) : null;
      if (rawTargetEntity === controllerEntity || (
        rawTargetEntity &&
        !isSalvageableWreckEntity(rawTargetEntity, salvagerRuntime)
      )) {
        continue;
      }
      addSalvageClaim(claims, currentID, -1);
      salvageCursor.reset();
      const result = assignByMode(drone, true);
      if (result === "assigned") {
        continue;
      }
      addSalvageClaim(claims, currentID, 1);
    ...
    for (const drone of idleDrones) {
      const result = assignByMode(drone, false);
      if (result === "stop") {
        return;
      }
    }
  ...
  function coordinationScopeKey(characterID, passContext) {
    const id = toInt(characterID, 0);
    const cache = passContext && passContext.fleetScopeByCharacter;
    if (cache instanceof Map && cache.has(id)) {
      return cache.get(id);
    }
    let scopeKey = "solo:" + id;
    try {
      const fleetRuntime = typeof deps.getFleetRuntime === "function"
        ? deps.getFleetRuntime()
        : null;
      const fleet = fleetRuntime && typeof fleetRuntime.getFleetForCharacter === "function"
        ? fleetRuntime.getFleetForCharacter(id)
        : null;
      const fleetID = toInt(fleet && (fleet.fleetID ?? fleet.fleetid), 0);
      if (fleetID > 0) {
        scopeKey = "fleet:" + fleetID;
      }
    } catch (error) {
      if (config.verbose) {
        log("fleet scope lookup failed for character " + id + ": " + (error && error.message));
      }
    }
    if (cache instanceof Map) {
      cache.set(id, scopeKey);
    }
    return scopeKey;
  }

  function claimsForScope(claimStore, scopeKey) {
    const key = String(scopeKey || "scope");
    if (!claimStore.has(key)) {
      claimStore.set(key, new Map());
    }
    return claimStore.get(key);
  }

  // Seed every managed target before any ship assigns a new one. This closes
  // the staggered-return gap where one hull's idle drone could take the rock
  // another hull's drone was already mining.
  function seedMiningClaims(scene, entries, passContext) {
    let itemTypeRegistry = null;
    try {
      itemTypeRegistry = deps.getItemTypeRegistry();
    } catch (error) {
      logError("mining claim seed could not read item types: " + (error && error.message));
      return;
    }
    for (const entry of entries) {
      try {
        const controllerEntity = scene.getEntityByID(entry.controllerID);
        if (!controllerEntity || entry.characterID <= 0) {
          continue;
        }
        const settings = getPlayerState(entry.characterID);
        const claims = claimsForScope(passContext.miningClaims, entry.scopeKey);
        for (const drone of entry.shipDrones) {
          if (
            classifyMiningDroneKind(drone, itemTypeRegistry) === null ||
            !isAutomatedMiningDrone(drone, settings)
          ) {
            continue;
          }
          addMiningClaim(claims, currentMiningTargetID(drone), 1);
        }
      } catch (error) {
        logError(
          "mining claim seed failed for controller " + entry.controllerID + ": " +
          (error && error.message),
        );
      }
    }
  ...
  function seedSalvageClaims(scene, entries, passContext) {
    let itemTypeRegistry = null;
    try {
      itemTypeRegistry = deps.getItemTypeRegistry();
    } catch (error) {
      logError("salvage claim seed could not read item types: " + (error && error.message));
      return;
    }
    const salvagerRuntime = safeSalvagerRuntime();
    for (const entry of entries) {
      try {
        ...
        const settings = getSalvageState(entry.characterID);
        const claims = claimsForScope(passContext.salvageClaims, entry.scopeKey);
        for (const drone of entry.shipDrones) {
          if (
            classifyDroneKind(drone, itemTypeRegistry, controllerEntity) !== "salvage" ||
            !isAutomatedSalvageDrone(drone, settings)
          ...
          addSalvageClaim(
            claims,
            activeSalvageTargetID(scene, controllerEntity, drone, salvagerRuntime),
            1,
          );
        }
      } catch (error) {
        logError(
          "salvage claim seed failed for controller " + entry.controllerID + ": " +
          (error && error.message),
        );
      }
    }
  ...
  function runScenePass(scene, nowMs) {
    ...
    // Everything in this object lasts exactly one scene pass. Fleet changes,
    // disconnected members and a disbanded fleet therefore need no cache
    // invalidation: the next pass simply resolves a new scope.
    const passContext = {
      fleetScopeByCharacter: new Map(),
      miningClaims: new Map(),
      salvageClaims: new Map(),
      salvageScans: new Map(),
    };
    const entries = [];
    for (const [controllerID, shipDrones] of byController) {
      const controllerEntity = scene.getEntityByID(controllerID);
      const characterID = resolveControllerCharacterID(controllerEntity);
      const scopeKey = characterID > 0
        ? coordinationScopeKey(characterID, passContext)
        : "solo:controller:" + controllerID;
      entries.push({ controllerID, shipDrones, characterID, scopeKey });
    }

    // Seed both kinds before the first assignment so every controller in a
    // fleet sees the same occupancy snapshot for this pass.
    seedMiningClaims(scene, entries, passContext);
    seedSalvageClaims(scene, entries, passContext);

    for (const entry of entries) {
      const miningClaims = claimsForScope(passContext.miningClaims, entry.scopeKey);
      try {
        runShipPass(
          scene, entry.controllerID, entry.shipDrones, nowMs, miningClaims,
        );
      } catch (error) {
        logError(
          `ship pass failed for controller ${entry.controllerID}: ${error && error.stack}`,
        );
      }
      const salvageClaims = claimsForScope(passContext.salvageClaims, entry.scopeKey);
      try {
        runSalvageShipPass(
          scene,
          entry.controllerID,
        ...
        logError(
          `salvage pass failed for controller ${entry.controllerID}: ${error && error.stack}`,
        );
      }
    }
  }
  ...
  // What the salvage drones are doing. Backs "/aud salvage status".
  ...
  return Object.freeze({
    ...
    // One kind's settings and nothing else: "/aud mining clear" must not throw the
    // mining queue away, and "/aud salvage clear" must not throw the salvage switches
    // away. The shared keys - radius, threshold, takeover - belong to neither
    // kind, so they stay until "/aud clear" clears the whole entry.
