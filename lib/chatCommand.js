...
// "/aud" on its own. Two menus, the two commands that cover a whole character
// rather than one kind of drone, and nothing else.
const ROOT_HELP_LINES = Object.freeze([
  "/aud m [command] - the mining drones; \"/aud m help\" lists what follows",
  "/aud s [command] - the salvage drones; \"/aud s help\" lists what follows",
  "/aud copy <name|id> - take another character's whole setup onto yours",
  "/aud reset - forget your personal settings and follow the server defaults",
  "/aud help - this list",
  "!aud <arguments> - the same thing from ordinary chat; no staff rights needed",
  "The kind comes first because the two squadrons are switched separately:",
  "\"/aud m off\" parks the miners and leaves the salvagers working, and",
  "\"/aud s off\" does the opposite.",
]);
...
const HELP_LINES = Object.freeze([
  ...
  "\"/aud m f ...\" is the same as \"/aud m filter ...\"",
...
const SALVAGE_HELP_LINES = Object.freeze([
  ...
  "    the far end",
  "/aud s foreign off|warn|allow - whose wrecks may be worked: only your own,",
  "    another pilot's with a warning line, or another pilot's in silence",
  "/aud s list - the wrecks around your ship, whose they are, and which you",
  "    may touch",
  ...
  "A wreck that is not yours is the game's question, not this mod's: the warning is",
  "printed once per launch, and your safety light still decides whether it may move.",
]);
...
function describeFilterUsage(head) {
  ...
  return "AdvancedUtilityDrones: filter takes add, move, del, clear, grade or fallback - " +
    'for example "/aud m filter add veldspar, kernite"; "/aud m filter" shows the queue, ' +
    'and "/aud m f" is the same command.';
}
...
function describeSalvageForeign(foreign) {
  if (foreign === "allow") {
    return "allow (another pilot's wreck is worked without a word)";
  }
  if (foreign === "warn") {
    return "warn (another pilot's wreck is worked, and the warning is printed once per launch)";
  }
  return "off (your own wrecks only)";
}

// The bay a salvage drone delivers into is the cargo hold - the server's own
// salvager cycle grants the material there, whatever specialized holds the hull
// was built with - so the line says so instead of leaving an expectation.
...
function buildSalvageStatusText(runtime, session) {
  ...
  const lines = [
    ...
    `  wrecks       : ${status.wrecks.total} in range - ${status.wrecks.mine} yours, ` +
      `${status.wrecks.theirs} another pilot's, ${status.wrecks.blocked} your light refuses`,
    `  foreign      : ${describeSalvageForeign(player.foreign)}`,
    `  assigned     : ${status.stats.salvageAssignments}, ` +
      `recalls ${status.stats.salvageRecalls}` +
      (status.stats.lastRecallReason ? ` (last: ${status.stats.lastRecallReason})` : ""),
    `  warnings     : ${status.stats.salvageWarnings} printed since the mod loaded`,
    `  threshold    : ${formatVolume(player.minHoldFreeVolumeM3)}`,
    `  takeover     : ${player.playerControlPolicy}`,
  ];
...
// "/aud s list" - every wreck worth flying to, with whose it is and whether this
// pilot may touch it. The order is the order the distance setting works them in,
// so the list reads as the run the drones are about to make.
function buildSalvageListText(runtime, session) {
  ...
  result.entries.forEach((entry, index) => {
    const whose = entry.entitled ? "yours to take" : `${entry.owner}'s`;
    const verdict = entry.blocked
      ? "your safety light refuses it"
      : (entry.entitled ? "no flag" : "salvaging it flags you");
    lines.push(
...
// "/aud s ..." - the salvage menu. The wreck questions the game answers
// elsewhere - whose wreck it is, whether the safety light lets the loot move -
// are switches here rather than guesses, because a salvage drone sent to the
// wrong wreck is a suspect flag, not a wasted cycle.
function handleSalvageCommand(runtime, config, session, argumentText) {
  ...
  if (action === "distance") {
    ...
    const normalized = normalizeCommandName(value);
    if (!["nearest", "farthest"].includes(normalized)) {
      return { message: 'AdvancedUtilityDrones: distance must be "nearest" or "farthest".' };
    }
    const state = runtime.setPlayerSalvageDistance(characterID, normalized, meta);
  ...
  if (action === "foreign") {
    if (!value) {
      ...
      return {
        message:
          `AdvancedUtilityDrones salvage foreign: ${describeSalvageForeign(current.foreign)}.`,
      };
    }
    const normalized = normalizeCommandName(value);
    if (!["off", "warn", "allow"].includes(normalized)) {
      return { message: 'AdvancedUtilityDrones: foreign must be "off", "warn" or "allow".' };
    }
    const state = runtime.setPlayerSalvageForeign(characterID, normalized, meta);
    return {
      message: `AdvancedUtilityDrones salvage foreign: ${describeSalvageForeign(state.foreign)}.`,
    };
  }
  ...
  return {
    message:
      `AdvancedUtilityDrones: unknown option "${action}". ` +
      'Use "/aud s help" for the list.',
  };
}
...
function handleCommand(runtime, config, session, argumentText) {
  ...
  if (MINING_SCOPES.includes(head)) {
    return handleMiningCommand(runtime, config, session, rest);
  }
  if (SALVAGE_SCOPES.includes(head)) {
...
function handleMiningCommand(runtime, config, session, argumentText) {
  ...
  if (ORE_FILTER_SCOPES.includes(action)) {
    return {
      message: `AdvancedUtilityDrones: "${action}" is a kind of rock, not a command - ` +
        `"/aud m filter clear ${action}" empties that list, and ` +
        `"/aud m list ${action}" shows what is around you.`,
    };
  }
...
function describeCopiedSettings(state, salvage, runtime) {
  ...
  if (salvage) {
    lines.push(
      ...
      `(${salvage.targetMode}, ${salvage.distance} first, foreign ${salvage.foreign})`,
    );
  }
  return lines;
}
