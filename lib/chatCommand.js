...
const RENAME_NOTICE =
  "AdvancedUtilityDrones was renamed: use \"/aud mining\" (short \"/aud mi\") for the " +
  "mining drones, or \"/aud salvage\" (short \"/aud sa\") for the salvage drones. " +
  "\"/atm\" and \"/altmining\" are gone.";
...
const SALVAGE_HELP_LINES = Object.freeze([
  "/aud salvage on|off - enable or disable automatic salvage for your character",
  "/aud salvage target spread|focus - one wreck per drone, or every drone on the same wreck",
  "/aud salvage distance nearest|farthest - work the field from the near end, or",
  "    the far end",
  "/aud salvage list - the wrecks around your ship, nearest first, and whose they are",
  "/aud salvage range [<meters|ship>] - show the search radius, or set it",
  "/aud salvage threshold <m3> - come home once the cargo hold has less room than this",
  "/aud salvage control hold|recall|off - what a manual order means for the mod",
  "/aud salvage resume - undo a manual takeover and let the mod fly those drones again",
  "/aud salvage clear - forget the salvage settings and follow the server defaults",
  "/aud salvage status - show the current salvage state",
  "/aud salvage help - this list",
  "Every command has two spellings, and the short one is two letters: salvage is",
  "    sa, and then sa on, sa off, sa tg, sa ds, sa rg, sa th, sa ls, sa ct,",
  "    sa rs, sa cl, sa st, sa h. Nothing else is accepted, and what a command",
  "    takes is always typed in full - spread, focus, nearest, farthest, ship,",
  "    hold, recall or off. \"/aud sa tg focus\" is \"/aud salvage target focus\".",
  "\"/aud salvage clear\" clears this kind alone; \"/aud clear\" clears both, and",
  "\"/aud help\" lists the two commands that cover the whole character",
...
// was meant, and this mod runs beside others on a shared server, so a letter
...
function buildHelpText(config, runtime = null) {
  const header = runtime
    ? `AdvancedUtilityDrones v${runtime.MOD_VERSION} - the commands that follow /aud mining:`
    : "AdvancedUtilityDrones - the commands that follow /aud mining:";
...
function buildSalvageHelpText(config, runtime = null) {
  const header = runtime
    ? `AdvancedUtilityDrones v${runtime.MOD_VERSION} salvage - the commands that follow /aud salvage:`
    : "AdvancedUtilityDrones salvage - the commands that follow /aud salvage:";
...
function buildFilterHelpText(config, runtime = null) {
  const header = runtime
    ? `AdvancedUtilityDrones v${runtime.MOD_VERSION} - the commands that follow /aud mining filter:`
    : "AdvancedUtilityDrones - the commands that follow /aud mining filter:";
  const lines = [
    header,
...
function buildStatusText(runtime, session) {
  ...
  lines.push('  use "/aud mining help" for the command list');
  return lines.join("\n");
}
...
const FILTER_HELP_LINES = Object.freeze([
  ...
  "    idle, a rock's name or its type ID.",
]);
...
function describeFilter(runtime, characterID) {
  ...
  lines.push("  commands: \"/aud mining filter help\" - add, move, del, clear, grade, fallback");
  return lines.join("\n");
}
...
function applyQueueMove(runtime, characterID, words, meta) {
  ...
  if (typed.usable.length === 0) {
    return {
      message: withKindLines(
        'AdvancedUtilityDrones: move needs a name - "/aud mining filter move veldspar 2".',
        runtime, current, notes,
      ),
    };
  }
...
function applyQueueDel(runtime, characterID, words, meta) {
  ...
  if (typed.usable.length === 0) {
    return {
      message: withKindLines(
        'AdvancedUtilityDrones: del needs a name - "/aud mining filter del kernite". ' +
...
function describeFilterUsage(head) {
  ...
  if (head === "list") {
    return 'AdvancedUtilityDrones: "/aud mining list" shows the rocks in range, ' +
...
function buildSalvageStatusText(runtime, session) {
  ...
  lines.push('  use "/aud salvage help" for the command list');
  return lines.join("\n");
}

// "/aud salvage list" - every wreck worth flying to, with whose it is. The order is the
...
function handleSalvageCommand(runtime, config, session, argumentText) {
  ...
  if (targetWord) {
    return {
      message: `AdvancedUtilityDrones: "${targetWord}" is a targeting mode, not a command - ` +
...
function handleCommand(runtime, config, session, argumentText) {
  ...
  return {
    message:
      `AdvancedUtilityDrones: start with a kind of drone - "/aud mi ${head}" for the ` +
      `mining drones, "/aud sa ${head}" for the salvage drones. "/aud help" lists the ` +
      "commands.",
  };
}
...
function handleTargetMode(runtime, characterID, kind, value, meta) {
  ...
  if (!value) {
    return {
      message:
        `AdvancedUtilityDrones ${kind} target: ${(read() || {}).targetMode || "spread"} ` +
        `(${what}).`,
    };
  }
  const asked = resolveWord(value, TARGET_MODE_WORDS);
  if (!asked) {
    return { message: 'AdvancedUtilityDrones: target must be "spread" or "focus".' };
  }
  const state = salvage
  ...
  return {
    message: salvage
      ? `AdvancedUtilityDrones salvage targeting mode: ${String(state.targetMode).toUpperCase()}.`
      : `AdvancedUtilityDrones targeting mode: ${String(state.targetMode).toUpperCase()}.`,
  };
}
