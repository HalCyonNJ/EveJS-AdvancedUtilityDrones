...
const OVERLAY_MARKER = Symbol.for("evejs.advancedUtilityDrones.chatOverlay");
const OVERLAY_VERSION = "1.0.0-alpha";
const {
...
// for good: the abbreviation is claimed by other mods on a shared server.
...
const RENAME_NOTICE =
  "AdvancedUtilityDrones was renamed: use \"/aud m\" for the mining drones or " +
  "\"/aud s\" for the salvage drones. \"/atm\" and \"/altmining\" are gone.";
...
const MINING_SCOPES = Object.freeze(["m", "mining", "mine", "miner", "miners"]);
const SALVAGE_SCOPES = Object.freeze(["s", "salvage", "salvager", "salvagers", "wreck", "wrecks"]);
// What can follow "/aud" itself: the two kinds of drone, and the two commands
// that cover a whole character. "c" is the copy and "r" the reset - no other
// root word wants either letter - and "/aud help" is answered like a bare "/aud".
const ROOT_WORDS = Object.freeze({
  mining: MINING_SCOPES,
  salvage: SALVAGE_SCOPES,
  copy: Object.freeze([]),
  reset: Object.freeze(["default", "r"]),
  help: Object.freeze(["h", "?"]),
});
// What can follow "/aud m". The letters in each list are the short spellings:
// "f" is the filter rather than the focus, "d" the reset rather than the range
// or the resume, and a word with no short spelling of its own is reached by the
// letters it still owns alone ("fa" is the fallback).
const MINING_ACTIONS = Object.freeze({
  on: Object.freeze([]),
  off: Object.freeze([]),
  spread: Object.freeze(["sp"]),
  focus: Object.freeze(["fo"]),
  range: Object.freeze(["ra"]),
  threshold: Object.freeze(["t"]),
  filter: Object.freeze(["f"]),
  list: Object.freeze(["l"]),
  control: Object.freeze(["c"]),
  resume: Object.freeze(["rs"]),
  reset: Object.freeze(["d", "default"]),
  status: Object.freeze(["st"]),
  fallback: Object.freeze([]),
  help: Object.freeze(["h", "?"]),
  copy: Object.freeze([]),
});
// What can follow "/aud s". "d" is the distance here rather than the reset,
// because on this menu the distance is the switch a pilot reaches for and the
// reset is not; "default" still spells the reset out.
const SALVAGE_ACTIONS = Object.freeze({
  on: Object.freeze([]),
  off: Object.freeze([]),
  spread: Object.freeze(["sp"]),
  focus: Object.freeze(["fo"]),
  distance: Object.freeze(["d"]),
  range: Object.freeze(["ra"]),
  threshold: Object.freeze(["t"]),
  list: Object.freeze(["l"]),
  control: Object.freeze(["c"]),
  resume: Object.freeze(["rs"]),
  reset: Object.freeze(["default"]),
  status: Object.freeze(["st"]),
  help: Object.freeze(["h", "?"]),
  copy: Object.freeze([]),
});
// What can follow "/aud m filter". Every verb here is picked out by its first
// letter on its own, which is why nothing longer is listed.
const FILTER_VERBS = Object.freeze({
  add: Object.freeze(["a", "set", "+"]),
  move: Object.freeze(["m", "mv"]),
  del: Object.freeze(["d", "delete", "remove", "rm", "-"]),
  clear: Object.freeze(["c", "empty"]),
  grade: Object.freeze(["g"]),
  fallback: Object.freeze([]),
  help: Object.freeze(["h", "?"]),
});
...
const RANGE_WORDS = Object.freeze({
  ship: Object.freeze(["auto", "default", "reset"]),
});
...
const ROOT_HELP_LINES = Object.freeze([
  "/aud m [command] - the mining drones; \"/aud m help\" lists every command",
  "/aud s [command] - the salvage drones; \"/aud s help\" lists every command",
]);
// "/aud m help" - what can follow "/aud m" and nothing else: the filter has a
// command list of its own, reached with "/aud m filter help", which is what
// keeps this one short enough to read in one screen.
const HELP_LINES = Object.freeze([
  "/aud m on|off - enable or disable automatic mining for your character",
  "/aud m spread|focus - one rock per drone, or every drone on the closest rock",
  "/aud m range [<meters|ship>] - show the search radius, or set it",
  "/aud m threshold <m3> - come home once the chosen hold has less room than this",
  "/aud m filter - show what to mine; \"/aud m filter help\" lists the rest",
  "/aud m list [ore|ice|moon] - what is mineable around your ship right now",
  "/aud m control hold|recall|off - what a manual order means for the mod",
  "/aud m resume - undo a manual takeover and let the mod fly those drones again",
  "/aud m reset - forget the mining settings and follow the server defaults",
  "/aud m status - show the current mining state",
  "/aud m help - this list",
  "\"/aud m f ...\" is the same as \"/aud m filter ...\": any word here answers",
  "    to the letters that still pick it out alone - \"/aud m sp\" is \"/aud m",
  "    spread\", \"/aud m t 500\" is \"/aud m threshold 500\", and a letter two",
  "    commands could take is answered with both of them",
  "\"/aud copy <name|id>\" takes a whole setup - both kinds - onto you, and",
  "\"/aud copy list\" shows who has settings stored here; \"/aud reset\" clears",
  "both kinds at once, where \"/aud m reset\" clears the mining half alone",
]);
// "/aud s help" - what can follow "/aud s" and nothing else. There is no
// salvage filter: every wreck in range is worked, in the order the distance
// setting asks for.
const SALVAGE_HELP_LINES = Object.freeze([
  "/aud s on|off - enable or disable automatic salvage for your character",
  "/aud s spread|focus - one wreck per drone, or every drone on the same wreck",
  "/aud s distance nearest|farthest - work the field from the near end, or",
  "    the far end",
  "/aud s list - the wrecks around your ship, nearest first, and whose they are",
  "/aud s range [<meters|ship>] - show the search radius, or set it",
  "/aud s threshold <m3> - come home once the cargo hold has less room than this",
  "/aud s control hold|recall|off - what a manual order means for the mod",
  "/aud s resume - undo a manual takeover and let the mod fly those drones again",
  "/aud s reset - forget the salvage settings and follow the server defaults",
  "/aud s status - show the current salvage state",
  "/aud s help - this list",
  "\"/aud copy <name|id>\" takes a whole setup - both kinds - onto you, and",
  "\"/aud reset\" clears both kinds at once",
  "Salvage material goes into the cargo hold, so the threshold is about cargo.",
  "A wreck is only a target: whoever it belonged to, it is the loot that carries a",
  "flag in this game and not the hull, so any wreck in range is worked.",
  "Any word here answers to the letters that still pick it out alone, so",
  "\"/aud s d farthest\" is \"/aud s distance farthest\".",
]);
...
// One slot of a command line, spelt out. Every table below maps the name a
// command answers to onto the other spellings that mean it, and a reading is
// tried in this order: the table's own short spellings first, because a short
// one is put there precisely because two names would otherwise share it, then
// the names and their spellings, and finally the letters they all start with
// when exactly one name is left. Two names left means nobody can guess which
// was meant, so the caller is handed both and says so rather than picking.
function resolveWord(word, words) {
  const token = normalizeCommandName(word);
  if (!token) {
    return { word: null, ambiguous: [] };
  }
  const names = Object.keys(words);
  for (const name of names) {
    if (words[name].includes(token)) {
      return { word: name, ambiguous: [] };
    }
  }
  for (const name of names) {
    if (name === token) {
  ...
  const hits = names.filter((name) => (
    name.startsWith(token) || words[name].some((alias) => alias.startsWith(token))
  ));
  if (hits.length === 1) {
    return { word: hits[0], ambiguous: [] };
  }
  return { word: null, ambiguous: hits.length > 1 ? hits : [] };
}

// "spread or status", for a reading that two commands would answer to.
function describeChoices(names) {
  if (names.length <= 1) {
    return names.join("");
  }
...
function describeUnknownWord(menu, typed, ambiguous) {
  if (ambiguous.length > 0) {
    return `AdvancedUtilityDrones: "${typed}" could be ${describeChoices(ambiguous)} - ` +
      `spell out the one you meant, or see "/aud ${menu} help".`;
  }
  return `AdvancedUtilityDrones: unknown option "${typed}". ` +
    `Use "/aud ${menu} help" for the list.`;
}
...
function buildHelpText(config, runtime = null) {
  const header = runtime
    ? `AdvancedUtilityDrones v${runtime.MOD_VERSION} - the commands that follow /aud m:`
    : "AdvancedUtilityDrones - the commands that follow /aud m:";
  ...
  if (!config.chatTrigger) {
    lines.push("  (the !aud chat trigger is disabled by the server configuration)");
  }
  return lines.join("\n");
}
...
// from the right one.
function buildRootHelpText(config, runtime = null) {
  const header = runtime
    ? `AdvancedUtilityDrones v${runtime.MOD_VERSION} - pick the drones to control:`
    : "AdvancedUtilityDrones - pick the drones to control:";
  const lines = [header, ...ROOT_HELP_LINES.map((line) => `  ${line}`)];
  if (!config.allowPlayerToggle) {
    lines.push("  (per-character control is disabled by the server configuration)");
  }
  if (!config.chatTrigger) {
...
// "/aud s help" - the salvage menu. Built like the mining one, so the two read
// the same way: one line per command, and the line is what to type.
function buildSalvageHelpText(config, runtime = null) {
  const header = runtime
    ? `AdvancedUtilityDrones v${runtime.MOD_VERSION} salvage - the commands that follow /aud s:`
    : "AdvancedUtilityDrones salvage - the commands that follow /aud s:";
...
// "/aud m filter help" - what can follow "/aud m filter", kept apart from "/aud m
...
function buildFilterHelpText(config, runtime = null) {
  const header = runtime
    ? `AdvancedUtilityDrones v${runtime.MOD_VERSION} filter - the commands that follow /aud m filter:`
    : "AdvancedUtilityDrones filter - the commands that follow /aud m filter:";
  const lines = [
    header,
...
function buildStatusText(runtime, session) {
  ...
  const lines = [
    `AdvancedUtilityDrones v${runtime.MOD_VERSION}`,
  ...
  lines.push('  use "/aud m help" for the command list');
  return lines.join("\n");
}
...
// "/aud m filter add gneiss, dark ochre" answers with the ore list, then the ice
...
// "/aud m filter help" - the filter's own commands, in the shape of the
...
const FILTER_HELP_LINES = Object.freeze([
  "/aud m filter add <name|id>[, <name|id> ...] - queue what to mine, first",
  ...
  "/aud m filter move <name|id> <place> - put one entry at a place in its own",
  "    list; the first name picks the list, and a name typed without a number",
  "    goes to the end",
  "/aud m filter del <name|id> - drop one entry; the name or its type ID",
  "    both work",
  "/aud m filter clear ore|ice|moon - empty one whole list",
  "/aud m filter grade on|off - mine the richest grade of a rock in range",
  "    before the plainer ones",
  "/aud m filter fallback any|idle - with nothing in range matching, mine",
  "    the closest rock anyway, or park the drones",
  "/aud m filter - show the queue, and the state of grade and fallback",
  "/aud m filter help - this list",
  "\"/aud m f <arguments>\" is the same as \"/aud m filter <arguments>\"",
]);
...
function buildOreListText(runtime, session, scope) {
  ...
  if (!result.hasShip) {
    return "AdvancedUtilityDrones: no active ship found.";
  }
  const what = scope ? `${scope} rocks` : "mineable rocks";
  if (result.entries.length === 0) {
    return `AdvancedUtilityDrones: no ${what} within ${formatMeters(result.rangeMeters)}.`;
  }
  const lines = [`AdvancedUtilityDrones ${what} in range (${formatMeters(result.rangeMeters)}):`];
  ...
  lines.push(`  queue one with: /aud m filter add <name>`);
  return lines.join("\n");
}

// "/aud m filter" - the queue as the reply prints it: one line per kind of rock
...
function describeFilter(runtime, characterID) {
  ...
  const lines = ["AdvancedUtilityDrones filter - the drones work down each list, first entry first:"];
  if (tokens.length === 0) {
  ...
  lines.push("  commands: \"/aud m filter help\" - add, move, del, clear, grade, fallback; \"f\" works");
  return lines.join("\n");
}
...
function handleFilterFallback(runtime, characterID, value, meta) {
  ...
  if (!normalized) {
    ...
    return {
      message:
        `AdvancedUtilityDrones filter fallback: ${state ? state.filterFallback : "any"} ` +
        '("any" mines the closest rock anyway once nothing in range matches the queue, ' +
        '"idle" leaves the drones parked instead).',
    };
  }
  const asked = resolveWord(normalized, FALLBACK_WORDS).word;
  if (!asked) {
    return { message: 'AdvancedUtilityDrones: fallback must be "any", "idle" or "default".' };
  }
  const state = runtime.setPlayerFilterFallback(
    characterID, asked === "default" ? null : asked, meta,
  );
  return { message: `AdvancedUtilityDrones filter fallback: ${state.filterFallback}.` };
}
// "/aud m filter grade on|off" - whether the richest grade of a rock in range is
...
function handleFilterGrade(runtime, characterID, value, meta) {
  const describe = (on) => (
    `AdvancedUtilityDrones filter grade: ${on ? "on" : "off"} ` +
    (on
      ? "(the richest grade of a rock in range is mined before the plainer ones)."
      : '(every grade counts the same - "filter grade on" prefers the richest).') +
  ...
  const asked = resolveWord(normalized, GRADE_WORDS).word;
  if (!asked) {
    return { message: 'AdvancedUtilityDrones: grade must be "on", "off" or "default".' };
  }
  ...
  return typed.reserved.map((word) => (
    oreQueue.isKindWord(word)
      ? `"${word}" names a whole kind of rock, so it is not an entry - ` +
        `/aud m filter clear ${word} empties the ${word} list`
...
// "/aud m filter add ..." - the queue grows by the names typed. An entry that is
...
function applyQueueAdd(runtime, session, characterID, words, meta) {
  ...
  if (refused.length > 0) {
    return { message: `AdvancedUtilityDrones: ${refused[0]}` };
  }
  const state = runtime.getPlayerState(characterID);
  ...
  const usable = typed.usable.filter((word) => {
    ...
    notes.push(
      ...
      "type ID, and only one that names a rock works; \"/aud m filter move <name> " +
  ...
  if (usable.length === 0) {
    return {
      message: withKindLines(
        'AdvancedUtilityDrones: add needs a name - ' +
        '"/aud m filter add veldspar, kernite, blue ice".',
  ...
  if (result.tokens.length > ORE_FILTER_MAX_PATTERNS) {
    return {
      message: `AdvancedUtilityDrones: the queue holds at most ${ORE_FILTER_MAX_PATTERNS} ` +
  ...
  if (result.skipped.length > 0) {
    ...
    notes.push(
      ...
      '"/aud m filter move <name> <place>" does',
    );
  }
  if (result.placed.length === 0) {
    return {
      message: withKindLines(
        "AdvancedUtilityDrones: nothing new to add " +
  ...
  return {
    message: withKindLines(
      `AdvancedUtilityDrones added ${added}.` +
...
// "/aud m filter move veldspar 2 kernite 1" - the queue is reordered with the
...
function applyQueueMove(runtime, characterID, words, meta) {
  ...
  const current = state && Array.isArray(state.oreFilter) ? state.oreFilter : [];
  if (current.length === 0) {
    return { message: "AdvancedUtilityDrones: the queue is empty, there is nothing to move." };
  }
  const typed = oreQueue.classifyTypedWords(words);
  const refused = describeRefusedWords(typed);
  if (refused.length > 0) {
  ...
  if (typed.usable.length === 0) {
    return {
      message: withKindLines(
        'AdvancedUtilityDrones: move needs a name - "/aud m filter move veldspar 2".',
        runtime, current, notes,
      ),
    };
  }
  ...
  if (kindsOfFirst.length > 1) {
    return {
      message: `AdvancedUtilityDrones: "${edits[0].token}" mines more than one kind of rock ` +
  ...
  if (result.placed.length === 0) {
    ...
    return {
      message: withKindLines(
        `AdvancedUtilityDrones: ${why} (the queue is ${describeFilterQueue(current, label)}).`,
        runtime, current, notes,
      ),
    };
  }
  ...
  return {
    message: withKindLines(
      `AdvancedUtilityDrones moved ${moved}${tail}.` + QUEUE_APPLIES_AT_ONCE,
      runtime,
      result.tokens,
      notes,
    ),
...
// "/aud m filter del kernite" - everything the typed names resolve to leaves the
...
function applyQueueDel(runtime, characterID, words, meta) {
  ...
  if (typed.usable.length === 0) {
    return {
      message: withKindLines(
        'AdvancedUtilityDrones: del needs a name - "/aud m filter del kernite". ' +
        '"/aud m filter clear ice" empties one kind of rock.',
        runtime, current, notes,
      ),
    };
  }
  ...
  if (removed.length === 0) {
    return {
      message: withKindLines(
        `AdvancedUtilityDrones: nothing in the queue matches ${missed.join(", ")} ` +
  ...
  return {
    message: withKindLines(
      `AdvancedUtilityDrones dropped ${dropped}${tail}.` +
        (tokens.length === 0
...
// "/aud m filter clear ice" - one kind's list is emptied. This is the only command
...
function applyQueueClear(runtime, characterID, words, meta) {
  ...
  const kind = resolveWord(typed, ROCK_KIND_WORDS).word || typed;
  if (!oreQueue.isKindWord(kind)) {
    return {
      message: 'AdvancedUtilityDrones: clear takes one kind of rock - ' +
        '"/aud m filter clear ore", "clear ice" or "clear moon".',
    };
  }
  const state = runtime.getPlayerState(characterID);
  const current = state && Array.isArray(state.oreFilter) ? state.oreFilter : [];
  ...
  if (result.removed.length === 0) {
    return {
      message: `AdvancedUtilityDrones: the ${kind} list is already empty.`,
    };
  }
  ...
  return {
    message: withKindLines(
      `AdvancedUtilityDrones dropped ${dropped} from the ${kind} list.` +
...
function describeFilterUsage(head) {
  if (ORE_FILTER_SCOPES.includes(head)) {
    return `AdvancedUtilityDrones: "${head}" is a kind of rock, not a command - ` +
      `"/aud m filter clear ${head}" empties that list, and ` +
      `"/aud m list ${head}" shows what is around you.`;
  }
  if (head === "list") {
    return 'AdvancedUtilityDrones: "/aud m list" shows the rocks in range, ' +
      '"/aud m list ice" just one kind.';
  }
  return "AdvancedUtilityDrones: filter takes add, move, del, clear, grade or fallback - " +
    'a, m, d, c, g or f if that is quicker. For example "/aud m f add veldspar, ' +
    'kernite", and "/aud m f" on its own shows the queue.';
}
...
function readScopeArgument(word) {
  ...
  const scope = resolveWord(word, ROCK_KIND_WORDS).word;
  if (!scope) {
    return { scope: null, error: 'AdvancedUtilityDrones: list takes "ore", "ice" or "moon".' };
  }
...
function sessionContext(session) {
  ...
  if (characterID <= 0) {
    return {
      characterID: 0,
      meta: null,
      error: "AdvancedUtilityDrones needs a character session.",
    };
  }
  return { characterID, meta: playerMeta(session), error: null };
}

// ─── /aud s: the salvage drones ─────────────────────────────────────────────
...
function buildSalvageStatusText(runtime, session) {
  ...
  const player = status.playerState;
  const lines = [
    `AdvancedUtilityDrones v${runtime.MOD_VERSION} salvage`,
  ...
  lines.push('  use "/aud s help" for the command list');
  return lines.join("\n");
}

// "/aud s list" - every wreck worth flying to, with whose it is. The order is the
// order the distance setting works them in, so the list reads as the run the
// drones are about to make.
function buildSalvageListText(runtime, session) {
  ...
  if (result.entries.length === 0) {
    return "AdvancedUtilityDrones: no salvageable wreck within " +
      `${formatMeters(result.rangeMeters)}.`;
  }
  const lines = [
    `AdvancedUtilityDrones wrecks in range (${formatMeters(result.rangeMeters)}):`,
  ];
  ...
  lines.push('  work them with: "/aud s on"');
  return lines.join("\n");
}

// "/aud s ..." - the salvage menu. There is no "whose wreck" switch: salvaging
...
function handleSalvageCommand(runtime, config, session, argumentText) {
  ...
  const requested = resolveWord(typed, SALVAGE_ACTIONS);
  const action = requested.word || typed;
  ...
  if (!config.allowPlayerToggle) {
    return { message: "AdvancedUtilityDrones: per-character control is disabled by the server." };
  }
  ...
  if (action === "on" || action === "off") {
    ...
    return {
      message:
        `AdvancedUtilityDrones salvage ${action.toUpperCase()} for you ` +
  ...
  if (action === "spread" || action === "focus") {
    const state = runtime.setPlayerSalvageTargetMode(characterID, action, meta);
    return {
      message:
        `AdvancedUtilityDrones salvage targeting mode: ${state.targetMode.toUpperCase()}.`,
    };
  }
  ...
  if (action === "distance") {
    if (!value) {
      ...
      return {
        message:
          `AdvancedUtilityDrones salvage distance: ${current.distance} first ` +
          '("nearest" works the field from the near end, "farthest" from the far end).',
      };
    }
    const direction = resolveWord(value, DISTANCE_WORDS).word;
    if (!direction) {
      return { message: 'AdvancedUtilityDrones: distance must be "nearest" or "farthest".' };
    }
    const state = runtime.setPlayerSalvageDistance(characterID, direction, meta);
    return { message: `AdvancedUtilityDrones salvage distance: ${state.distance} first.` };
  }
  ...
  return { message: describeUnknownWord("s", typed, requested.ambiguous) };
}

// ─── the commands that sit above a kind of drone ────────────────────────────

// "/aud reset": the whole entry, both kinds and the shared keys with it.
function handleResetAll(runtime, characterID) {
  runtime.clearPlayerSettings(characterID);
  return {
    message:
      "AdvancedUtilityDrones: all your personal settings were cleared - both kinds of " +
      "drone, and the shared radius, threshold and takeover setting with them; the " +
      'server defaults apply again. "/aud m reset" and "/aud s reset" clear one kind ' +
      "on its own.",
  };
}

// "/aud m reset" and "/aud s reset": one kind's keys, and nothing else. The
// shared keys are not one kind's to throw away, so they stay until "/aud reset".
function handleKindReset(runtime, characterID, kind, meta) {
  ...
  return {
    message:
      `AdvancedUtilityDrones: the ${what} settings were cleared; the server defaults ` +
      `apply again (automation ${state && state.enabled ? "ON" : "OFF"}, ` +
      `mode ${state ? state.targetMode : "spread"}). The shared radius, threshold and ` +
      'takeover setting are untouched - "/aud reset" clears everything.',
  };
}
...
function handleResume(runtime, session, asked) {
  ...
  return {
    message: resumed > 0
      ? `AdvancedUtilityDrones: ${resumed} ${what} handed back to the automation.`
      : `AdvancedUtilityDrones: no ${what} of yours is under manual control.`,
  };
}
...
function handleThreshold(runtime, characterID, value, meta) {
  if (!value) {
    ...
    return {
      message:
        `AdvancedUtilityDrones threshold: ${formatVolume(current.minHoldFreeVolumeM3)} ` +
  ...
  if (!Number.isFinite(numeric) || numeric < 0 || numeric > MAX_THRESHOLD_M3) {
    return {
      message:
        "AdvancedUtilityDrones threshold must be a number of cubic metres between " +
  ...
  return {
    message:
      `AdvancedUtilityDrones threshold: ${formatVolume(state.minHoldFreeVolumeM3)}. ` +
...
function handleControl(runtime, characterID, value, meta) {
  if (!value) {
    ...
    return {
      message:
        `AdvancedUtilityDrones manual takeover: ${current.playerControlPolicy} ` +
  ...
  const asked = resolveWord(value, CONTROL_WORDS).word;
  if (!asked) {
    return { message: 'AdvancedUtilityDrones control must be "hold", "recall" or "off".' };
  }
  const state = runtime.setPlayerControlPolicy(characterID, asked, meta);
  return {
    message: `AdvancedUtilityDrones manual takeover: ${state.playerControlPolicy.toUpperCase()}.`,
  };
}
...
function handleCommand(runtime, config, session, argumentText) {
  ...
  return {
    message:
      `AdvancedUtilityDrones: start with a kind of drone - "/aud m ${head}" for the ` +
      `mining drones, "/aud s ${head}" for the salvage drones. "/aud help" lists the ` +
      "commands.",
  };
}
// "/aud m ..." - the mining menu, unchanged from 1.3.0 apart from the name
...
function handleMiningCommand(runtime, config, session, argumentText) {
  ...
  const typed = normalizeCommandName(parts[0] || "status");
  const value = parts.slice(1).join(" ").trim();
  const requested = resolveWord(typed, MINING_ACTIONS);
  const action = requested.word || typed;
  ...
  if (action === "filter") {
    ...
    const verb = resolveWord(typedVerb, FILTER_VERBS).word;
  ...
  if (requested.ambiguous.length > 0) {
    return { message: describeUnknownWord("m", typed, requested.ambiguous) };
  }
  const rockKind = resolveWord(typed, ROCK_KIND_WORDS).word;
  if (rockKind) {
    return {
      message: `AdvancedUtilityDrones: "${rockKind}" is a kind of rock, not a command - ` +
        `"/aud m f clear ${rockKind}" empties that list, and ` +
        `"/aud m l ${rockKind}" shows what is around you.`,
    };
  }
  ...
  if (action === "on" || action === "off") {
    ...
    return {
      message:
        `AdvancedUtilityDrones ${action.toUpperCase()} for you ` +
  ...
  if (action === "spread" || action === "focus") {
    ...
    return {
      message: `AdvancedUtilityDrones targeting mode: ${state.targetMode.toUpperCase()}.`,
    };
  }
...
// typed after "list".
//
// A character becomes findable the first time they run any /aud m command,
...
function describeCopyListing(runtime, characterID, words) {
  ...
  if (all.length === 0) {
    return {
      message: "AdvancedUtilityDrones copy list - nobody is stored here yet: set one " +
  ...
  if (asked && matching.length === 0) {
    return {
      message: `AdvancedUtilityDrones: nobody stored here matches "${asked}". ` +
        '"/aud copy list" shows everyone.',
    };
  }
  const lines = [
    asked
      ? `AdvancedUtilityDrones copy list "${asked}" - ${matching.length} of ${all.length} ` +
        "character(s) match:"
      : `AdvancedUtilityDrones copy list - ${all.length} character(s) stored here:`,
  ];
...
function handleCopy(runtime, config, characterID, value, meta) {
  if (!config.allowPlayerCopy) {
    return {
      message: "AdvancedUtilityDrones: copying another character's settings is disabled " +
        "by the server.",
    };
  }
  ...
  if (!found.entry) {
    if (found.error === "ambiguous") {
      return {
        message: `AdvancedUtilityDrones: "${asked.raw}" fits more than one character - ` +
          `${describeCopySource(entries, found.candidates)}. Type more of the name, or ` +
          "the character ID.",
      };
    }
    return {
      message: `AdvancedUtilityDrones: no character matching "${asked.raw}" has settings ` +
        'stored here. "/aud copy list" shows everyone who does.',
    };
  }
  if (found.characterID === characterID) {
    return {
      message: "AdvancedUtilityDrones: that is you, so there is nothing to copy. " +
  ...
  if (!copySettings.hasCopyableSettings(found.entry)) {
    ...
    return {
      message: `AdvancedUtilityDrones: ${label} has nothing set, so your settings were ` +
  ...
  if (!state) {
    return {
      message: "AdvancedUtilityDrones: the copy could not be stored - check the server log.",
    };
  }
  return {
    message: `AdvancedUtilityDrones: copied ${label} onto you.\n` +
...
function handleRange(runtime, session, characterID, value, meta) {
  if (!value) {
    ...
    return {
      message: range
        ? `AdvancedUtilityDrones search radius: ${formatMeters(range.rangeMeters)} ` +
          `(${describeBreakdown(range.breakdown)})`
        : "AdvancedUtilityDrones search radius: no active ship found.",
    };
  }
  if (resolveWord(value, RANGE_WORDS).word === "ship") {
    runtime.setPlayerRangeOverride(characterID, null, meta);
    const status = runtime.describeStatus(session);
    return {
      message: "AdvancedUtilityDrones search radius now follows your ship: " +
  ...
  if (!Number.isFinite(numeric) || numeric < MIN_RANGE_METERS || numeric > MAX_RANGE_METERS) {
    return {
      message:
        "AdvancedUtilityDrones range must be a number between " +
        `${MIN_RANGE_METERS} and ${MAX_RANGE_METERS} meters, or "ship".`,
    };
  }
  runtime.setPlayerRangeOverride(characterID, Math.round(numeric), meta);
  return {
    message: `AdvancedUtilityDrones search radius set to ${formatMeters(numeric)} ` +
      '(use "/aud m range ship" to follow the ship again).',
  };
}
...
function install(upstream, options) {
  ...
  if (helpDescriptor && helpDescriptor.writable) {
    ...
    upstream.COMMANDS_HELP_TEXT =
      `${help}\n${ROOT_HELP_LINES.join("\n")}\n${HELP_LINES.join("\n")}\n` +
      `${SALVAGE_HELP_LINES.join("\n")}`;
  }
